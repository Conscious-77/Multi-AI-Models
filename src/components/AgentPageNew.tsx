...
const AgentPageNew: React.FC = () => {
  ...
  const [pdfMode, setPdfMode] = useState<'auto' | 'always' | 'never'>('auto');
    ...
    try {
      const response = await fetch('/api/agent2', {
        ...
        body: JSON.stringify({ 
          ...
          config: agentConfig,
          options: { pdf: pdfMode, filename: pdfFilename }
        })
      });
      if (!response.ok) throw new Error(`API 请求失败: ${response.status}`);
      const data = await response.json();
      let content = data.response || '';
      if (data.toolResult && data.toolResult.result && data.toolResult.result.url) {
        const url = data.toolResult.result.url;
        const size = data.toolResult.result.bytes;
        content += `\n\n已生成文件：[下载PDF](${url})` + (size ? `（${size} bytes）` : '');
      }
      setMessages(prev => prev.map(msg => msg.id === assistantMessage.id ? { ...msg, content, isLoading: false } : msg));
    } catch (error) {
        ...
        <div className={`config-panel ${showConfig ? 'open' : ''}`}>
          ...
          <div className="config-section">
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <select value={pdfMode} onChange={(e) => setPdfMode(e.target.value as any)}>
                <option value="auto">自动</option>
                <option value="always">总是生成</option>
                <option value="never">从不生成</option>
              </select>
              <input 
                type="text" 
                placeholder="文件名（可选，含 .pdf 或自动补全）"
                value={pdfFilename}
